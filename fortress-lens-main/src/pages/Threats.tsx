...
import { ShieldAlert, ShieldX, Shield, Eye, RefreshCw, AlertTriangle, ShieldCheck } from "lucide-react";
...
export default function Threats() {
  ...
  const { data: rawData = [], isLoading: loading, refetch: fetchThreats } = useQuery({
    queryKey: ["threats"],
    queryFn: () => getThreats(200),
    staleTime: 30_000,
  });

  const threats = useMemo(() => (rawData as Record<string, unknown>[]).map(norm), [rawData]);
    ...
    <AppLayout title="Threats" breadcrumb={["Firewall Analytics"]}>
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-card rounded-xl p-4 shadow-card">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Critical Threats</span>
        ...
        <div className="bg-card rounded-xl p-4 shadow-card">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">High Risk</span>
        ...
        <div className="bg-card rounded-xl p-4 shadow-card">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Total Detected</span>
      ...
      {threats.length === 0 ? (
        <div className="bg-card rounded-xl p-16 shadow-card text-center">
          ...
          <p className="text-sm font-medium text-foreground mb-2">No threats yet</p>
          <p className="text-xs text-muted-foreground">Upload a traffic CSV log to view detected threats.</p>
        </div>
      ) : (
        <>
              ...
              <div className={`rounded-xl p-5 mb-6 border ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 ${
                    ...
                    {isCritical ? <ShieldAlert className="h-6 w-6 text-red-400" /> : isWarning ? <AlertTriangle className="h-6 w-6 text-yellow-400" /> : <ShieldCheck className="h-6 w-6 text-emerald-400" />}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div className="bg-card rounded-xl p-5 shadow-card">
              <h3 className="text-sm font-semibold text-foreground mb-4">Threats by Severity</h3>
            ...
            <div className="bg-card rounded-xl p-5 shadow-card">
              <h3 className="text-sm font-semibold text-foreground mb-4">Threat Type Distribution</h3>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
          ...
          <div className="bg-card rounded-xl p-5 shadow-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={fetchThreats} disabled={loading}>
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="bg-card border-border max-w-md">
          <DialogHeader><DialogTitle className="text-sm font-semibold">{selected?.name}</DialogTitle></DialogHeader>
          {selected && (
            <div className="space-y-3 mt-2">
              ...
              <div className="rounded-lg p-3 mb-1" style={{
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground uppercase tracking-wider font-bold">Risk Score</span>
