    ...
    <div className="fixed right-0 top-0 h-full w-80 bg-card border-l border-border shadow-2xl z-50 overflow-y-auto">
      <div className="flex items-center justify-between p-4 border-b border-border sticky top-0 bg-card">
        <div>
          ...
          <p className="text-xs text-muted-foreground font-mono">{node.ip_address}</p>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Zone Details</p>
            ...
            <div className="space-y-1.5">
              {inbound.map((e) => (
                <div key={e.id} className="bg-secondary/30 rounded p-2 text-xs">
                  ...
                  {e.reason && <p className="text-warning/80 mt-0.5 text-xs">{e.reason}</p>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              {outbound.map((e) => (
                <div key={e.id} className="bg-secondary/30 rounded p-2 text-xs">
                  ...
                  {e.recommendation && (
                    <p className="text-success/80 mt-0.5 text-xs">{e.recommendation}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {denied.length > 0 && (
          <div>
            ...
            <div className="space-y-1.5">
              {denied.map((e) => (
                <div key={e.id} className="bg-destructive/10 rounded p-2 text-xs">
  ...
  return (
    <div className="bg-card rounded-xl p-5 shadow-card mb-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Full Attack Graph</h3>
            ...
            <div key={i} className="flex items-center gap-0">
                ...
                <div className="flex flex-col items-center mx-1">
                  ...
                  {hop && (
                    <div className="text-center mt-0.5">
                      ...
                      <p className="text-xs" style={{ color: riskHex(hopRisk) }}>
                        {hopRisk.toFixed(1)}
                      </p>
                    </div>
                  )}
              ...
              >
                <span className="mb-0.5">
                  {isTarget ? <Target className="h-4 w-4 text-red-400" /> : isEntry ? <Globe className="h-4 w-4 text-blue-400" /> : <Lock className="h-4 w-4 text-muted-foreground" />}
                </span>
                <p className="text-xs font-medium text-foreground leading-tight">
                  {node.replace(/_/g, " ")}
                </p>
                {isEntry  && <p className="text-xs text-blue-400 mt-0.5">ENTRY</p>}
                {isTarget && <p className="text-xs text-red-400 mt-0.5">TARGET</p>}
              </div>
            </div>
          );
        })}
  ...
  return (
    <AppLayout title="Attack Paths" breadcrumb={["Firewall Analytics"]}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Attack Path Analysis</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
      ...
      {loading ? (
        <div className="flex items-center gap-3 text-sm text-muted-foreground py-20 justify-center">
        ...
        <>
              ...
              <div className={`rounded-xl p-5 mb-6 border ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 ${
                    ...
                    {isCritical ? <ShieldAlert className="h-6 w-6 text-red-400" /> : isHigh ? <AlertTriangle className="h-6 w-6 text-orange-400" /> : isClean ? <ShieldCheck className="h-6 w-6 text-emerald-400" /> : <SearchIcon className="h-6 w-6 text-yellow-400" />}
                  </div>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <div className="bg-card rounded-xl p-4 shadow-card">
              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">Critical Paths</p>
              <p className="text-2xl font-bold tabular-nums text-red-500">{summary?.critical_paths_count ?? 0}</p>
            </div>
            <div className="bg-card rounded-xl p-4 shadow-card">
              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">High Risk Paths</p>
              <p className="text-2xl font-bold tabular-nums text-orange-400">{summary?.high_risk_paths_count ?? 0}</p>
            </div>
            <div className="bg-card rounded-xl p-4 shadow-card">
              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">Avg Risk Score</p>
              <p className="text-2xl font-bold tabular-nums text-primary">{num(summary?.average_path_risk).toFixed(1)}</p>
            </div>
            <div className="bg-card rounded-xl p-4 shadow-card">
              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">Connections</p>
              <p className="text-2xl font-bold tabular-nums text-foreground">{graph?.stats.total_edges ?? 0}</p>
              <p className="text-xs text-muted-foreground">{graph?.stats.high_risk_edges ?? 0} high-risk</p>
            </div>
          </div>
          ...
          {paths.length > 0 && (() => {
            ...
            return (
              <div className="bg-card rounded-xl p-5 shadow-card mb-6">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h3 className="text-sm font-semibold text-foreground">Path Risk Distribution</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">
              ...
              <div className="bg-card rounded-xl p-5 shadow-card mb-6 border border-red-900/30 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs uppercase tracking-widest text-red-400/80 font-bold mb-1">Highest Priority Attack Path</p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className={`${riskLabel(topPath.risk_level)} px-2 py-0.5 rounded-full text-xs font-semibold uppercase mt-1 inline-block`}>
                      {topPath.risk_level}
                    </span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-1 mb-6 bg-card rounded-xl p-1 shadow-card w-fit">
            {([["graph", "Attack Graph", <Zap className="h-3.5 w-3.5" />], ["paths", "Attack Paths", <AlertCircle className="h-3.5 w-3.5" />]] as const).map(([id, label, icon]) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-medium transition-smooth ${
            ...
            <>
              {paths.length === 0 ? (
                <div className="bg-card rounded-xl p-10 shadow-card text-center mb-6">
                  <p className="text-xs text-muted-foreground">No attack paths were found between entry points and targets.</p>
                </div>
              ) : (
                <div className="space-y-4 mb-6">
                  {paths.map((path) => (
                    <div key={path.id} className="bg-card rounded-xl shadow-card overflow-hidden">
                      ...
                      >
                        <span
                          className="text-lg font-bold tabular-nums shrink-0 cursor-help"
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className={`${riskLabel(path.risk_level)} px-2 py-0.5 rounded-full text-xs font-semibold uppercase`}>
                            {path.risk_level}
                          </span>
                          {expanded === path.id
                      ...
                      {expanded === path.id && (
                        <div className="px-4 pb-5 pt-0 border-t border-border">
                          ...
                          <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mt-3 mb-3">
                            Attack Flow
                          </p>
                          <PathFlow path={path} />
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-4">
                            {path.weakest_link && (
                              <div>
                                <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">Weakest Rule</p>
                                <SmartTooltip term={path.weakest_link} context="Weakest link in attack path" severity={path.risk_level} page="AttackPaths">
                                  <p className="text-xs text-warning font-mono cursor-help">{path.weakest_link}</p>
                                </SmartTooltip>
                              </div>
                            )}
                            {path.vulnerable_ports?.length > 0 && (
                              <div>
                                <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-1">Vulnerable Ports</p>
                                <div className="flex flex-wrap gap-1">
                                  {path.vulnerable_ports.map((port) => (
                                    <SmartTooltip key={port} term={`Port ${port}`} context={`Vulnerable port in attack path`} severity={path.risk_level} page="AttackPaths">
                                      <span className="text-xs bg-destructive/10 text-destructive px-1.5 py-0.5 rounded font-mono cursor-help">
                                        :{port}
                                      </span>
                                    </SmartTooltip>
                                  ))}
                          ...
                          {Array.isArray(path.path_hops) && path.path_hops.length > 0 && (
                            <div className="mt-4">
                              <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-2">Hop Details</p>
                      ...
                      <div key={ep.node_id} className="bg-card rounded-xl p-4 shadow-card flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <p className="text-xs font-medium text-foreground truncate">{ep.device_name}</p>
                            <span className="badge-critical px-1.5 py-0.5 rounded text-xs font-semibold shrink-0">
                              Internet Exposed
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground">
