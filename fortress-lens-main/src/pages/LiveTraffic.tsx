...
import { Search, Download, RefreshCw, AlertTriangle, ChevronLeft, ChevronRight, X, Activity, ShieldAlert, ShieldCheck } from "lucide-react";
...
const ActionBadge = ({ action }: { action: string }) => {
  ...
  return <span className={`${cls} px-2 py-0.5 rounded text-xs font-semibold capitalize`}>{action}</span>;
};

const DetailField = ({ label, value }: { label: string; value: string | number | undefined }) => (
  <div><p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-0.5">{label}</p>
  <p className="text-xs font-mono text-foreground">{value ?? "—"}</p></div>
);
...
export default function LiveTraffic() {
  ...
  const { data: rawData = [], isLoading: loading, refetch: fetchRows } = useQuery({
    queryKey: ["liveTraffic"],
    queryFn: () => getConnections(500),
    staleTime: 30_000,
    refetchInterval: autoRefresh ? 10_000 : false,
  });

  const traffic = useMemo(() => (rawData as Record<string, unknown>[]).map(mapRow), [rawData]);
  ...
  return (
    <AppLayout title="Live Traffic" breadcrumb={["Firewall Analytics"]}>
      <div className="flex items-center gap-3 p-3 bg-card rounded-xl shadow-card mb-6">
        ...
        <Button variant="outline" size="sm" className="text-xs h-8 ml-auto" onClick={fetchRows} disabled={loading}>
          <RefreshCw className={`h-3.5 w-3.5 mr-1.5 ${loading ? "animate-spin" : ""}`} />{loading ? "Loading…" : "Refresh"}
        </Button>
        <Button variant="outline" size="sm" className="text-xs h-8" onClick={exportCSV} disabled={!traffic.length}>
      ...
      {traffic.length === 0 && !loading ? (
        <div className="bg-card rounded-xl p-16 shadow-card text-center">
          ...
          <p className="text-sm font-medium text-foreground mb-2">No traffic data yet</p>
          <p className="text-xs text-muted-foreground">Upload a traffic CSV log to populate this view.</p>
        </div>
      ) : (
        <>
            ...
            <div className={`rounded-xl p-5 mb-6 border ${
              ...
              <div className="flex items-center gap-4">
                <div className={`h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 ${
                  ...
                  {isCritical ? <ShieldAlert className="h-6 w-6 text-red-400" /> : deniedCount > 0 ? <AlertTriangle className="h-6 w-6 text-yellow-400" /> : <ShieldCheck className="h-6 w-6 text-emerald-400" />}
                </div>
        ...
        <div className="bg-card rounded-xl shadow-card overflow-hidden">
              ...
              <thead>
                <tr className="border-b border-border">
                  {["Time","Source","Destination","Protocol","App","User","Data Sent","Data Received","Action",""].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-xs uppercase tracking-widest text-muted-foreground font-bold whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={e.id} onClick={() => setSelected(e)}
                    ...
                    <td className="px-4 py-2.5">
                      <SmartTooltip term={e.protocol} context="Network protocol" page="LiveTraffic">
                        <span className="bg-secondary px-1.5 py-0.5 rounded text-xs font-mono cursor-help">{e.protocol}</span>
                      </SmartTooltip>
                    </td>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            <span className="text-xs text-muted-foreground">{filtered.length} connections</span>
        ...
        <DialogContent className="bg-card border-border max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle className="text-sm font-semibold">Connection Detail</DialogTitle>
            ...
            <div className="grid grid-cols-2 gap-4 mt-2">
              ...
              {selected.threatDetected && (
                <div className="col-span-2 bg-destructive/10 border border-destructive/20 rounded-lg p-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-destructive font-medium">Threat detected on this connection — investigate immediately</span>
                </div>
              )}
            </div>
          )}
