...
import {
  ...
  ChevronRight, ArrowUpRight, ArrowDownRight, Minus, Radio,
  ShieldCheck, Globe, Wrench,
} from "lucide-react";
...
import {
  ...
  getSyslogStatus, startSyslog, stopSyslog,
  type Threat, type SyslogStatus,
} from "@/lib/api";
...
const SeverityBadge = ({ severity }: { severity: string }) => {
  ...
  return <span className={`${cls} px-2 py-0.5 rounded-full text-xs font-semibold uppercase`}>{severity}</span>;
};
...
export default function Dashboard() {
  ...
  const syslogQ        = useQuery({ queryKey: ["syslogStatus"],   queryFn: getSyslogStatus,          staleTime: LIVE_STALE,   refetchInterval: 5_000 });
  ...
  const syslogStatus  = syslogQ.data        ?? null;
  ...
  const refetchAll = useCallback(() => {
    ...
    queryClient.invalidateQueries({ queryKey: ["syslogStatus"] });
  }, [queryClient]);

  const [syslogToggling, setSyslogToggling] = useState(false);
  const handleSyslogToggle = async () => {
    setSyslogToggling(true);
    try {
      ...
      console.error("Syslog toggle failed:", e);
    } finally {
      setSyslogToggling(false);
    }
  };
    ...
    <AppLayout title="Dashboard" breadcrumb={["Firewall Analytics"]}>
      ...
      {fwTopology?.chain_detected && (
        <div className="bg-info/10 border border-info/20 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-info mb-1">Firewall Chain Detected</p>
            <p className="text-xs text-muted-foreground">{fwTopology.chain_details}</p>
            {fwTopology.firewalls.length > 1 && (
              <div className="flex gap-2 mt-2">
                {fwTopology.firewalls.map((fw) => (
                  <span key={fw.device_name} className="text-xs bg-info/10 text-info px-2 py-0.5 rounded font-medium">
      ...
      <div className="bg-card rounded-xl p-4 shadow-card mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {hasData && (
              <>
                <Button variant="outline" size="sm" className="h-8 text-xs" onClick={downloadPDFReport}>
                  <FileText className="h-3.5 w-3.5 mr-1.5" />Export PDF
                </Button>
                <Button variant="outline" size="sm" className="h-8 text-xs" onClick={downloadCSVExport}>
                  <Download className="h-3.5 w-3.5 mr-1.5" />Export CSV
                </Button>
              </>
            )}
            <button onClick={refetchAll} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-smooth ml-2">
      ...
      {!hasData ? (
        <div className="bg-card rounded-xl p-16 shadow-card text-center">
          ...
          <p className="text-sm font-semibold text-foreground mb-2">No data yet</p>
          <p className="text-xs text-muted-foreground mb-6">Upload a firewall config to populate all analytics — no mock data is shown.</p>
        ...
        <>
              ...
              <div className={`rounded-xl p-5 mb-6 border ${
                ...
                <div className="flex items-center gap-5">
                  <div className={`h-16 w-16 rounded-2xl flex items-center justify-center shrink-0 ${
                    ...
                    {isCritical ? <ShieldAlert className="h-7 w-7 text-red-400" /> : isModerate ? <AlertTriangle className="h-7 w-7 text-yellow-400" /> : <ShieldCheck className="h-7 w-7 text-emerald-400" />}
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="flex gap-3 shrink-0">
                    {[
                      ...
                      <div key={m.label} className="text-center px-4 py-2 bg-black/20 rounded-xl">
                        ...
                        <p className="text-xs text-muted-foreground uppercase tracking-wider mt-0.5">{m.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              { label: "Attack Paths", desc: `${n(attackSurface?.critical_paths)} critical paths`, Icon: Target, path: "/attack-paths",
                color: n(attackSurface?.critical_paths) > 0 ? "border-red-900/30 hover:border-red-700/40" : "border-border hover:border-primary/30" },
              { label: "Remediation", desc: `${n(risk?.by_level?.critical) + n(risk?.by_level?.high)} issues to fix`, Icon: Wrench, path: "/remediation",
              ...
              { label: "FW Topology", desc: `${n(topology?.firewalls_count)} firewall${n(topology?.firewalls_count) !== 1 ? "s" : ""}`, Icon: Globe, path: "/firewall-topology",
                color: "border-border hover:border-primary/30" },
              { label: "Threats", desc: `${n(risk?.by_level?.critical)} critical threats`, Icon: Shield, path: "/threats",
              ...
              <button key={nav.label} onClick={() => navigate(nav.path)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-foreground">{nav.label}</p>
          ...
          {execSummary && (
            <div className="bg-card rounded-xl p-5 shadow-card mb-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">Auto-Generated from Your Data</span>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">{execSummary.summary}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-card rounded-xl p-5 shadow-card">
              <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Overall Risk Score</span>
              <div className="flex items-center gap-3 mt-3">
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-muted-foreground capitalize">{execSummary?.risk_trend ?? "stable"}</span>
                  </div>
                  <span className="text-xs text-muted-foreground">Industry avg: 45</span>
                </div>
              </div>
            ...
            <div className="bg-card rounded-xl p-5 shadow-card">
              <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Firewall Health Score</span>
              <div className="flex items-center gap-3 mt-3">
            ...
            {/* Attack Surface */}
            <div className="bg-card rounded-xl p-5 shadow-card">
              <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Attack Surface</span>
              {attackSurface ? (
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div>
                    <p className="text-xl font-bold text-warning tabular-nums">{attackSurface.exposed_ports}</p>
                    <p className="text-xs text-muted-foreground">Exposed Ports</p>
                  </div>
                  <div>
                    <p className="text-xl font-bold text-destructive tabular-nums">{attackSurface.internet_facing_rules}</p>
                    <p className="text-xs text-muted-foreground">Internet-Facing Rules</p>
                  </div>
                  <div>
          ...
          {/* Syslog Collection Status */}
          <div className="bg-card rounded-xl p-5 shadow-card mb-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Radio className="h-4 w-4 text-primary" />
                <h3 className="text-sm font-semibold text-foreground">Syslog Collection</h3>
              ...
              <Button
                variant={syslogStatus?.running ? "destructive" : "default"}
                size="sm"
                className="h-7 text-xs"
                disabled={syslogToggling}
                onClick={handleSyslogToggle}
              >
            ...
            {syslogStatus?.running && (
              <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                <span>UDP :{syslogStatus.udp_port}</span>
                <span>TCP :{syslogStatus.tcp_port}</span>
                <span>Uptime: {Math.floor((syslogStatus.uptime_seconds ?? 0) / 60)}m {Math.floor((syslogStatus.uptime_seconds ?? 0) % 60)}s</span>
                <span>Connections: {(syslogStatus.connections_inserted ?? 0).toLocaleString()}</span>
                <span>Threats: {(syslogStatus.threats_inserted ?? 0).toLocaleString()}</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-card rounded-xl p-5 shadow-card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-foreground">Top 5 Critical Findings</h3>
                <Button variant="ghost" size="sm" className="h-6 text-xs text-primary" onClick={() => navigate("/analysis")}>
              ...
              {execSummary?.top_findings?.length ? (
                <div className="space-y-2">
                  {execSummary.top_findings.map((f, i) => (
                    <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-primary/5 transition-smooth">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-foreground truncate">{f.rule_name}</p>
                ...
                <div className="h-32 flex items-center justify-center text-xs text-muted-foreground">No critical findings detected</div>
              )}
            </div>
                  ...
                  {compliance.map((c) => (
                    <div key={c.framework}>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs font-medium text-foreground">{c.framework}</span>
                        <div className="flex items-center gap-2">
                          <span className={`text-sm font-bold tabular-nums ${complianceStatusColor(c.status)}`}>
                            {c.score.toFixed(0)}%
                          </span>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{c.findings} finding{c.findings !== 1 ? "s" : ""} · {c.details[0]}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="h-32 flex items-center justify-center text-xs text-muted-foreground">Upload a config to see compliance scores</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div className="bg-card rounded-xl p-5 shadow-card">
              ...
              {protocolData.length > 0 ? (
                ...
                <div className="h-60 flex items-center justify-center text-xs text-muted-foreground">Upload a config to see protocol breakdown</div>
              )}
            </div>

            <div className="bg-card rounded-xl p-5 shadow-card">
              <h3 className="text-sm font-semibold text-foreground mb-4">Topology Summary</h3>
              {donutData.length > 0 ? (
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                ...
                <div className="h-60 flex items-center justify-center text-xs text-muted-foreground">No topology data yet</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-card rounded-xl p-5 shadow-card">
              <h3 className="text-sm font-semibold text-foreground mb-4">Risk Level Breakdown</h3>
              {risk ? (
                ...
                <div className="h-40 flex items-center justify-center text-xs text-muted-foreground">Run rule analysis to see risk breakdown</div>
              )}
            </div>

            <div className="bg-card rounded-xl p-5 shadow-card">
              <h3 className="text-sm font-semibold text-foreground mb-4">Recent Threats</h3>
              {normThreats.length === 0 ? (
                <div className="h-40 flex items-center justify-center text-xs text-muted-foreground">No threats detected yet</div>
              ) : (
                    ...
                    <thead>
                      <tr className="border-b border-border text-left">
                        {["Time", "Threat", "Severity", "Source"].map((h) => (
                          <th key={h} className="pb-2 text-xs uppercase tracking-widest text-muted-foreground font-bold">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
