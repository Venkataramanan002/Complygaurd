    ...
    <div className="mt-2 rounded-xl border border-destructive/20 bg-destructive/5 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">— Risk playbook for {findingKey}</span>
        </div>
            ...
            {[
              ...
              <div key={key} className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs uppercase tracking-widest text-destructive/70 font-bold mb-0.5">{label}</p>
                    ...
                    <ol className="space-y-1">
                      {list.map((item, i) => (
                        <li key={i} className="text-xs text-foreground leading-relaxed flex gap-1.5">
                          <span className="text-destructive/60 font-mono text-xs mt-px">{i + 1}.</span>
                          <span>{item}</span>
                        </li>
                      ))}
                    </ol>
