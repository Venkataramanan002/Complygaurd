...
# Dedicated parsers for switches and routers
SWITCH_PARSER = CiscoSwitchParser()
ROUTER_PARSER = CiscoRouterParser()
...
DATA_IMPORT_EXTENSIONS = (".csv", ".json", ".xlsx", ".xls")


def detect_upload_vendor(filename: str) -> str:
    lowered = filename.lower()
    if filename.endswith(".xml"):
        return "paloalto"
    if "switch" in lowered or "sw-" in lowered or "sw_" in lowered:
        return "switch"
    if "router" in lowered or "rtr" in lowered or "rt-" in lowered or "rt_" in lowered:
        return "router"
    if "forti" in lowered:
        return "fortinet"
    if lowered.endswith(".conf") or "asa" in lowered:
        return "cisco"
    if any(lowered.endswith(ext) for ext in DATA_IMPORT_EXTENSIONS):
        return "universal"
    raise HTTPException(
        status_code=400,
        detail="Unsupported file. Supported: .xml (Palo Alto), .conf (Cisco ASA/switch/router), forti*.conf (FortiGate), switch*.conf, router*.conf, .csv, .json, .xlsx (traffic data)"
    )


async def process_single_upload_file(
    file: UploadFile,
    background_tasks: BackgroundTasks,
    db: AsyncSession,
) -> Dict[str, Any]:
    filename = file.filename or ""
    vendor = detect_upload_vendor(filename)
    ...
    if vendor == "universal":
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Data ingestion failed for {filename}: {str(e)}")
    ...
    if len(content) > max_upload_bytes:
        raise HTTPException(
            status_code=413,
            detail=f"{filename}: file too large. Maximum is {max_upload_bytes // (1024 * 1024)} MB."
        )

    text_content = content.decode("utf-8", errors="ignore")
    device_name = None
    if vendor == "switch":
        from parsers.switch_parser import CiscoSwitchParser as _SP
        device_name = _SP()._parse_hostname(text_content)
    elif vendor == "router":
        from parsers.router_parser import CiscoRouterParser as _RP
        device_name = _RP()._parse_hostname(text_content)
    else:
        device_name = f"{vendor}-fw-01"

    if device_name:
        device_rule_ids = select(FirewallRule.id).where(FirewallRule.device_name == device_name)
        await db.execute(delete(CertificationReview).where(
            CertificationReview.rule_id.in_(device_rule_ids)
        ))
        await db.execute(delete(RuleOwner).where(
            RuleOwner.rule_id.in_(device_rule_ids)
        ))
        await db.execute(delete(RuleRiskAnalysis).where(
            RuleRiskAnalysis.rule_id.in_(device_rule_ids)
        ))
        await db.execute(delete(FirewallRule).where(FirewallRule.device_name == device_name))
        await db.execute(delete(NetworkTopology).where(NetworkTopology.device_name == device_name))
        await db.execute(delete(Connection).where(Connection.source == "config_projection").where(Connection.device_name == device_name))
        await db.execute(delete(Threat).where(Threat.source == "config_projection").where(Threat.device_name == device_name))
...
async def upload_config(
    ...
    for upload in upload_list:
        results.append(await process_single_upload_file(upload, background_tasks, db))
    ...
    if len(results) == 1:
        ...
        return {
            ...
            "processed_rows": single.get("processed_rows"),
            "errors_count": single.get("errors_count"),
        }
...
async def process_config_background(upload_id: str, content: str = ""):
    async with AsyncSessionLocal() as db:
        try:
            ...
            # ── Detect parser type ───────────────────────────────────────
            is_switch = upload.vendor == "switch"
            is_router = upload.vendor == "router"
            is_firewall = not is_switch and not is_router

            if is_switch:
                # ── SWITCH processing path ──────────────────────────────
                parsed = SWITCH_PARSER.parse(content)
                device_name = parsed["device_name"]
                topology_data_raw = SWITCH_PARSER.to_topology_nodes(parsed)
                rules_data = []  # Switches don't have firewall rules (ACLs handled separately)

                # Extract ACL-based rules if any
                for acl in parsed.get("acls", []):
                    for i, ace in enumerate(acl.get("rules", [])):
                        rules_data.append({
                            "device_name": device_name,
                            "rule_name": f"{acl['name']}-{i+1}",
                            "rule_position": i + 1,
                            "source_ip": "any",
                            "source_port": "any",
                            "dest_ip": "any",
                            "dest_port": "any",
                            "protocol": "ip",
                            "action": "allow" if ace.get("action") == "permit" else "deny",
                            "is_enabled": True,
                        })

            elif is_router:
                # ── ROUTER processing path ──────────────────────────────
                parsed = ROUTER_PARSER.parse(content)
                device_name = parsed["device_name"]
                topology_data_raw = ROUTER_PARSER.to_topology_nodes(parsed)
                rules_data = ROUTER_PARSER.to_firewall_rules(parsed)

            else:
                # ── FIREWALL processing path (existing) ─────────────────
                ...
                topology_data_raw = None  # will be parsed below

            # ── Step 1: Store rules ──────────────────────────────────────
            rules_objs = []
            for r in rules_data:
                rule = FirewallRule(
                    device_name=r.get("device_name", device_name),
            ...
            # ── Step 2: Parse and store topology ─────────────────────────
            if is_firewall and topology_data_raw is None:
                topology_data_raw = parser.parse_topology(content)

            topo_objs = []
            for t in (topology_data_raw or []):
                topo = NetworkTopology(
                    device_name=t.get("device_name", device_name),
                    ...
                    # Switch-specific
                    vlans=t.get("vlans"),
                    trunk_ports=t.get("trunk_ports"),
                    access_ports=t.get("access_ports"),
                    stp_mode=t.get("stp_mode"),
                    stp_root_for=t.get("stp_root_for"),
                    port_security=t.get("port_security"),
                    # Router-specific
                    interfaces=t.get("interfaces"),
                    routing_protocol=t.get("routing_protocol"),
                    ospf_area=t.get("ospf_area"),
                    bgp_asn=t.get("bgp_asn"),
                    bgp_neighbors=t.get("bgp_neighbors"),
                    static_routes=t.get("static_routes"),
                    nat_rules=t.get("nat_rules"),
                    acls=t.get("acls"),
                    # Link info
                    link_type=t.get("link_type"),
                    link_speed=t.get("link_speed"),
                    neighbor_device=t.get("neighbor_device"),
                    neighbor_port=t.get("neighbor_port"),
                )
            ...
            await db.commit()
            topology_data = topology_data_raw or []
            ...
            if is_firewall:
                ...
                # For switches/routers, use the base parser's derive method with the ACL rules
                from parsers.config_parsers import BaseConfigParser
                _base = BaseConfigParser()
                synthetic = _base.derive_synthetic_data(rules_data, topology_data, device_name)
            ...
            # DFS to find paths (capped to prevent memory exhaustion)
            MAX_PATHS = 10000
            found_paths = []
            ...
            if len(found_paths) >= MAX_PATHS:
                logger.warning(f"Attack path limit reached ({MAX_PATHS}). Results truncated.")
...
async def get_risky_rules(min_score: float = 0.0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    limit = min(limit, 500)  # SECURITY: cap unbounded limit
    try:
        stmt = (
...
async def get_connections(limit: int = 200, db: AsyncSession = Depends(get_db)):
    limit = min(limit, 1000)  # SECURITY: cap unbounded limit
    try:
...
async def get_vulnerable_ports(db: AsyncSession = Depends(get_db)):
    try:
        ...
        for port, meta in VULNERABLE_PORTS.items():
            ...
            for rule, analysis in rule_rows:
                from utils.risk_engine import resolve_port
                dp = resolve_port(rule.dest_port) if rule.dest_port else None
