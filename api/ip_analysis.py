...
# PAN-OS service object names → port number (module-level for reuse)
_SVC_PORT_MAP: Dict[str, int] = {
    "service-http": 80, "svc-http": 80, "service-https": 443, "svc-https": 443,
    "service-ssh": 22, "svc-ssh": 22, "service-ftp": 21, "svc-ftp": 21,
    "service-dns": 53, "svc-dns": 53, "service-smtp": 25, "svc-smtp": 25,
    "service-rdp": 3389, "svc-rdp": 3389, "service-smb": 445, "svc-smb": 445,
    "service-telnet": 23, "svc-telnet": 23, "service-mysql": 3306, "svc-mysql": 3306,
    "service-postgresql": 5432, "svc-postgresql": 5432, "service-mssql": 1433, "svc-mssql": 1433,
    "service-ldap": 389, "svc-ldap": 389, "service-ldaps": 636, "svc-ldaps": 636,
    "service-snmp": 161, "svc-snmp": 161, "service-ntp": 123, "svc-ntp": 123,
    "service-radius": 1812, "svc-radius": 1812, "service-syslog": 514, "svc-syslog": 514,
    "service-mqtt": 1883, "svc-mqtt": 1883, "service-modbus": 502, "svc-modbus": 502,
    "service-ipsec": 500, "svc-ipsec": 500, "service-ipsec-nat": 4500,
    "service-http-alt": 8080, "svc-http-alt": 8080,
    "service-dr-replication": 8443, "service-kerberos": 88,
    "service-backup": 9102, "service-zabbix": 10051, "service-dr-heartbeat": 7946,
    "service-oracle": 1521, "service-rsh": 514, "service-netbios": 139,
    "service-imap": 143, "service-tftp": 69, "service-icmp": 0,
    "application-default": 443,
    "svc-mongodb": 27017, "service-mongodb": 27017,
    "svc-redis": 6379, "service-redis": 6379,
    "svc-elasticsearch": 9200, "service-elasticsearch": 9200,
}
...
def _port_details(port_str: str) -> Tuple[float, str, List[str]]:
    ...
    # Try resolving service name first (e.g., "svc-mysql" → 3306)
    resolved = _SVC_PORT_MAP.get(port_str.strip().lower())
    if resolved is not None:
        info = _PORT_INFO.get(resolved)
        if info:
    ...
    svc_name = _SVC_PORT_MAP.get(port_str.strip().lower())
    display = f"port {svc_name}" if svc_name else f"port {port_str}"
    return 5.0, (
        f"Unknown service on {display}. "
...
def _ip_in_rule(ip: str, rule_ip_field: str, zone_subnets: Optional[Dict[str, str]] = None) -> bool:
    """Return True when `ip` is covered by a rule's source/dest field.

    Supports: literal IPs, CIDRs, wildcards (any/all/*), and zone names
    (when zone_subnets mapping is provided).
    """
    ...
    # Zone name → subnet mapping fallback
    if zone_subnets:
        subnet_str = zone_subnets.get(raw)
        if subnet_str:
            try:
                return ipaddress.ip_address(ip) in ipaddress.ip_network(subnet_str, strict=False)
...
async def analyze_ip_vulnerability(
    ...
    # ── Build zone→subnet mapping from topology ─────────────────────────
    zone_subnets: Dict[str, str] = {}
    for n in topo_nodes:
        z = (n.zone or "").strip().lower()
        ip_raw = (n.ip_address or "").strip()
        if z and ip_raw and z not in zone_subnets:
            try:
                addr = ipaddress.ip_address(ip_raw)
                parts = str(addr).split(".")
                if len(parts) == 4:
                    zone_subnets[z] = f"{parts[0]}.{parts[1]}.{parts[2]}.0/24"
            except (ValueError, TypeError):
                pass
    ...
    source_found = source_ip in all_known or any(_ip_in_rule(source_ip, k, zone_subnets) for k in all_known)
    target_found = target_ip in all_known or any(_ip_in_rule(target_ip, k, zone_subnets) for k in all_known)
    ...
    for rule in all_rules:
        ...
        fwd = _ip_in_rule(source_ip, rule.source_ip or "", zone_subnets) and _ip_in_rule(target_ip, rule.dest_ip or "", zone_subnets)
        rev = _ip_in_rule(target_ip, rule.source_ip or "", zone_subnets) and _ip_in_rule(source_ip, rule.dest_ip or "", zone_subnets)
    ...
    for fw in firewall_nodes:
        ...
        for rule, _ in matching_rules:
            if (_ip_in_rule(fw_ip, rule.source_ip or "", zone_subnets) or _ip_in_rule(fw_ip, rule.dest_ip or "", zone_subnets)):
...
class AttackSurfaceRequest(BaseModel):
    source_ip: str = Field(..., max_length=45)
    target_ip: str = Field(..., max_length=45)
    # SECURITY: csv_path/xml_path REMOVED — user-supplied file paths are an
    # arbitrary-file-read vulnerability.  The backend uses its own defaults.
...
async def analyze_attack_surface(
    ...
    """
    Attack surface analysis between two IPs.

    Primary data source: firewall rules in the database (from uploaded configs).
    Fallback: static CSV + XML files when available.
    """
    csv_path = _DEFAULT_CSV_PATH
    xml_path = _DEFAULT_XML_PATH

    # Prefer DB rules — build a temporary CSV-like interaction list from uploaded rules
    src = req.source_ip.strip()
    tgt = req.target_ip.strip()

    # Build zone → subnet mapping from topology so we can match zone names to IPs
    topo_q = await db.execute(select(NetworkTopology))
    topo_nodes = topo_q.scalars().all()
    zone_subnets: Dict[str, str] = {}
    for node in topo_nodes:
        if node.zone and node.ip_address:
            # Derive /24 subnet from the zone's IP (e.g. 10.10.10.1 → 10.10.10.0/24)
            try:
                ip_obj = ipaddress.ip_address(node.ip_address)
                subnet = ipaddress.ip_network(f"{ip_obj}/24", strict=False)
                zone_subnets[node.zone.lower()] = str(subnet)
            except (ValueError, TypeError):
                pass

    # Service name → port mapping (reuse module-level map)
    _SERVICE_PORTS = _SVC_PORT_MAP

    def _ip_matches_zone_or_addr(ip_str: str, rule_field: str) -> bool:
        """Check if an IP matches a rule's source/dest which can be a zone name, CIDR, or 'any'."""
        field = rule_field.strip().lower()
        if field == "any":
            return True
        # Direct IP/CIDR match
        try:
            net = ipaddress.ip_network(rule_field, strict=False)
            return ipaddress.ip_address(ip_str) in net
        except (ValueError, TypeError):
            pass
        # Zone name match — look up the zone's subnet
        subnet_str = zone_subnets.get(field)
        if subnet_str:
            try:
    ...
    result_q = await db.execute(select(FirewallRule).where(FirewallRule.is_enabled == True))
    db_rules = result_q.scalars().all()

    if db_rules:
        import tempfile, csv as csv_mod
        interactions = []
        for r in db_rules:
            src_match = _ip_matches_zone_or_addr(src, r.source_ip or "")
            dst_match = _ip_matches_zone_or_addr(tgt, r.dest_ip or "")

            if src_match and dst_match:
                port_raw = (r.dest_port or "any").strip().lower()
                port_int = _SERVICE_PORTS.get(port_raw, None)
                if port_int is None:
                    try:
                        port_int = int(str(port_raw).split("-")[0].split("/")[0])
                    except (ValueError, TypeError):
                        port_int = 0
                interactions.append({
                    "source_ip": src, "destination_ip": tgt,
                    "port": port_int, "protocol": r.protocol or "tcp",
                    "access_type": r.action or "allow",
                })

        if interactions:
            tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False, newline="")
            writer = csv_mod.DictWriter(tmp, fieldnames=["source_ip", "destination_ip", "port", "protocol", "access_type"])
            writer.writeheader()
