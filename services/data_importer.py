    ...
    try:
        if file_type == 'csv':
            ...
            if vendor != 'standard':
                ...
                # Auto-detect dataset type by trying each schema in order
                cols_lower = [c.lower() for c in df.columns]
                records = df.to_dict(orient='records')

                if 'threat_name' in cols_lower and 'severity' in cols_lower:
                    data['threats'] = records
                elif 'src_port' in cols_lower and 'dst_port' in cols_lower and 'action' in cols_lower:
                    data['connections'] = records
                elif 'rule_name' in cols_lower and 'device_name' in cols_lower:
                    data['firewall_rules'] = records
                elif 'device_type' in cols_lower and 'zone' in cols_lower:
                    data['network_topology'] = records
                elif 'cpu_usage_percent' in cols_lower:
                    data['system_health'] = records
                else:
                    # Last resort: try connections, if missing columns just put in threats
                    conn_cols = {'src_port', 'dst_port', 'protocol', 'action'}
                    if conn_cols.issubset(set(cols_lower)):
                        data['connections'] = records
                    else:
                        data['threats'] = records
        elif file_type == 'json':
...
async def ingest_data_file(file: UploadFile, db: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        if cleaned['connections']:
            for row in cleaned['connections']:
